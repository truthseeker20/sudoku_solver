...
int main(int argc, char* argv[]) {
    ...
    if (order<=0) {
        printf("Error: Order must be a positive perfect square number.\n");
        return 1;
    }
    ...
    if ((root*root) != order) {
        printf("Error: %d is not a perfect square.\n",order);
        return 1;
    }

    //int board [order][order] = {0};
    ...
    if (isSolved) {printBoard(board,order);}
